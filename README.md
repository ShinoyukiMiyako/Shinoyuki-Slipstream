...
| `requireClient` | `false` | 强制双端：开启后服务端在登录期拒绝未启用其全部协商优化的客户端。开启 = 放弃 vanilla 兼容换取强制省流，供大型服务器保证全员启用优化 |

协商类功能（zstd / 聚合 / 实体 delta / 大包）的能力通道在启动时注册，因此开关这些选项需要重启服务端（及客户端）。

## 兼容性

- **原版/未装客户端**：可正常连接；所有优化对其回退为原版行为。例外：服务端开启 `requireClient` 后，未安装或未启用对应优化的客户端会在登录期被拒绝连接。
